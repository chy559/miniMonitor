...
- Rich tray menu with status details, copy/export summary, manual refresh, quota refresh, Task Manager and Resource Monitor shortcuts, high-usage alerts with adjustable threshold, global hotkey toggle, background low-frequency refresh, pause/resume, refresh interval, opacity control, window lock, always-on-top, position reset, app folder shortcut, settings reset, startup-hidden toggle, and auto-start toggle
