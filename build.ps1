...
g++ -std=c++17 -O2 -Wall -Wextra -municode -mwindows -static -static-libgcc -static-libstdc++ `
  "$Root\src\main.cpp" "$Root\src\app_window.cpp" "$Root\src\format_utils.cpp" "$Root\src\system_sampler.cpp" "$Root\src\codex_quota_client.cpp" "$Root\src\ui_theme.cpp" "$ResFile" `
  -o "$ExeFile" `
